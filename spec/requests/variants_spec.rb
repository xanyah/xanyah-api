  ...
  describe "GET /variants" do
   ...
   describe "PATCH /variants/:id" do
     it "updates variant if membership" do
       ...
       patch variant_path(variant), params: {variant: {ratio: new_variant.ratio} }, headers: user.create_new_auth_token
       expect(response).to have_http_status(:ok)
     ...
     it "returns 401 if !membership" do
       new_variant = build(:variant)
       patch variant_path(create(:variant)), params: { variant: {ratio: new_variant.ratio} }, headers: create(:user).create_new_auth_token
       expect(response).to have_http_status(:unauthorized)
     end
