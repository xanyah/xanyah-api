...
RSpec.describe "CustomAttributes", type: :request do
  ...
  describe "PATCH /custom_attributes/:id" do
    it "updates custom_attribute if membership" do
      ...
      patch custom_attribute_path(custom_attribute), params: {custom_attribute: {name: new_custom_attribute.name} }, headers: user.create_new_auth_token
      expect(response).to have_http_status(:ok)
    ...
    it "returns 401 if !membership" do
      new_custom_attribute = build(:custom_attribute)
      patch custom_attribute_path(create(:custom_attribute)), params: { custom_attribute: {name: new_custom_attribute.name} }, headers: create(:user).create_new_auth_token
      expect(response).to have_http_status(:unauthorized)
    end

    it "returns 401 if !loggedin" do
      new_custom_attribute = build(:custom_attribute)
      patch custom_attribute_path(create(:custom_attribute)), params: { custom_attribute: {name: new_custom_attribute.name} }
      expect(response).to have_http_status(:unauthorized)
      expect(JSON.parse(response.body)).to have_key('errors')
    end
  end
