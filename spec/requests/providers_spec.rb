...
RSpec.describe "Providers", type: :request do
  ...
  describe "PATCH /providers/:id" do
    it "updates provider if membership" do
      ...
      patch provider_path(provider), params: {provider: {name: new_provider.name} }, headers: user.create_new_auth_token
      expect(response).to have_http_status(:ok)
    ...
    it "returns 401 if !membership" do
      new_provider = build(:provider)
      patch provider_path(create(:provider)), params: { provider: {name: new_provider.name} }, headers: create(:user).create_new_auth_token
      expect(response).to have_http_status(:unauthorized)
    end
