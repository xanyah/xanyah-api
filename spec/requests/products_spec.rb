  ...
  describe "GET /products" do
   ...
   describe "PATCH /products/:id" do
     it "updates product if membership" do
       ...
       patch product_path(product), params: {product: {name: new_product.name} }, headers: user.create_new_auth_token
       expect(response).to have_http_status(:ok)
     ...
     it "returns 401 if !membership" do
       new_product = build(:product)
       patch product_path(create(:product)), params: { product: {name: new_product.name} }, headers: create(:user).create_new_auth_token
       expect(response).to have_http_status(:unauthorized)
     end
