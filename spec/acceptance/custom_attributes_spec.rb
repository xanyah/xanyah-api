...
resource 'Custom Attributes' do
  ...
  route '/custom_attributes', 'Custom Attributes collection' do
    ...
    post 'Create a custom attribute' do
      with_options scope: :custom_attribute do
        parameter :name, "Custom Attribute's name", required: true
        parameter :type, "Custom Attribute's type (text, number)", required: true
        parameter :store_id, "Custom Attribute's store id", required: true
      end
  ...
  route '/custom_attributes/:id', 'Single custom attribute' do
    let!(:custom_attribute) { create(:custom_attribute, store: membership.store) }

    with_options scope: :custom_attribute do
      parameter :name, "Custom Attribute's name", required: true
      parameter :type, "Custom Attribute's type", required: true
    end
    ...
    delete 'Delete a specific custom attribute' do
      ...
      example_request 'Deleting a custom attribute' do
        ...
        expect(CustomAttribute.where(id: id).size).to eq(0)
      end
    end
  end
end
